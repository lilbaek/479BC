...
{
    jni_function_handler handler;
    if (jni_get_method_handler(CLASS_game_ACTIVITY, "showDirectorySelection", "(Z)V", &handler)) {
...
float android_get_screen_density(void)
{
    jni_function_handler handler;
    float result = 1.0f;
    if (jni_get_method_handler(CLASS_game_ACTIVITY, "getScreenDensity", "()F", &handler)) {
...
int android_get_file_descriptor(const char *filename, const char *mode)
{
    ...
    if (!jni_get_static_method_handler(CLASS_FILE_MANAGER, "openFileDescriptor",
        "(Lcom/github/Keriew/479BC/479BCMainActivity;Ljava/lang/String;Ljava/lang/String;)I", &handler)) {
        jni_destroy_function_handler(&handler);
        return 0;
    }
...
int android_get_directory_contents(const char *dir, int type, const char *extension, int (*callback)(const char *))
{
    ...
    if (!jni_get_static_method_handler(CLASS_FILE_MANAGER, "getDirectoryFileList",
        "(Lcom/github/Keriew/479BC/479BCMainActivity;Ljava/lang/String;ILjava/lang/String;)[Ljava/lang/String;",
        &handler)) {
        jni_destroy_function_handler(&handler);
        return LIST_ERROR;
    }
...
int android_remove_file(const char *filename)
{
    ...
    if (!jni_get_static_method_handler(CLASS_FILE_MANAGER, "deleteFile",
        "(Lcom/github/Keriew/479BC/479BCMainActivity;Ljava/lang/String;)Z", &handler)) {
        jni_destroy_function_handler(&handler);
        return 0;
    }
    jstring jfilename = (*handler.env)->NewStringUTF(handler.env, filename);
...
JNIEXPORT void JNICALL Java_com_github_Keriew_game_479BCMainActivity_gotDirectory(JNIEnv *env, jobject thiz)
{
    has_directory = 1;
}
