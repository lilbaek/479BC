...
static int map_can_place_initial_road(int grid_offset) {
    if (terrain_land_citizen.items[grid_offset] == CITIZEN_N1_BLOCKED) {
        // not open land (blocked by building etc)
        return 0;
...
int map_routing_calculate_distances_for_building(routed_building_type type, int x, int y) {
    ...
    if (!map_can_place_initial_road(source_offset)) {
        return 0;
    }
    if (map_terrain_is(source_offset, TERRAIN_ROAD) &&
        type != ROUTED_BUILDING_ROAD) {
        return 0;
    }
    ++stats.total_routes_calculated;
...
static int callback_delete_wall(int next_offset, int dist, int direction) {
    if (terrain_land_citizen.items[next_offset] < CITIZEN_0_ROAD) {
        if (map_terrain_is(next_offset, TERRAIN_WALL)) {
...
void map_routing_delete_first_wall(int x, int y) {
    ++stats.total_routes_calculated;
    route_queue_all_from(map_grid_offset(x, y), DIRECTIONS_NO_DIAGONALS, callback_delete_wall, 0);
}
...
static inline int has_fighting_enemy(int grid_offset) {
    if (!(fighting_data.status.items[grid_offset] & 0x40)) {
        fighting_data.status.items[grid_offset] |=
                0x40 | (map_figure_foreach_until(grid_offset, is_fighting_enemy) << 1);
    }
...
static int callback_travel_noncitizen_land_through_building(int offset, int next_offset, int direction) {
    ...
    if (terrain == NONCITIZEN_1_BUILDING &&
        (map_building_id == state.through_building_id || map_building_id == state.dest_building_id)) {
        return 1;
    }
    return 0;
}
...
int map_routing_noncitizen_can_travel_over_land(
    ...
    if (only_through_building_id) {
        ...
        route_queue_from_to(src_x, src_y, dst_x, dst_y, num_directions, 0,
                            callback_travel_noncitizen_land_through_building);
    } else {
...
int
map_routing_noncitizen_can_travel_through_everything(int src_x, int src_y, int dst_x, int dst_y, int num_directions) {
    ++stats.total_routes_calculated;
