...
static void determine_cartpusher_destination(figure *f, building *b, int road_network_id) {
    ...
    if (city_resource_is_stockpiled(b->output_resource_id) || building_stockpiling_enabled(b)) {
        dst_building_id = building_warehouse_for_storing(0, f->x, f->y,
                                                         b->output_resource_id, road_network_id, &understaffed_storages,
                                                         &dst);
    }
    if (dst_building_id) {
    ...
    dst_building_id = building_granary_for_storing(f->x, f->y,
                                                   b->output_resource_id, road_network_id, 0, &understaffed_storages,
                                                   &dst);
    int distFarm = 0;
    building *src_building_farm = building_get(f->building_id);
    building *dst_building_farm = building_get(dst_building_id);
    int src_building_type_farm = src_building_farm->type;
    if ((src_building_type_farm >= BUILDING_WHEAT_FARM && src_building_type_farm <= BUILDING_PIG_FARM) ||
        src_building_type_farm == BUILDING_WHARF) {
        distFarm = calc_maximum_distance(src_building_farm->x, src_building_farm->y, dst_building_farm->x,
                                         dst_building_farm->y);
    }
    if (distFarm >= 64) {
        dst_building_id = 0;
    ...
    dst_building_id = building_granary_for_storing(f->x, f->y,
                                                   b->output_resource_id, road_network_id, 1, &understaffed_storages,
                                                   &dst);
    int dist = 0;
