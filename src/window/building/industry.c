...
void city_mint_conversion_changed(int accepted)
{
    if (!accepted) {
        return;
    }
...
static void set_city_mint_conversion(int resource, int param2)
{
    if (building_get(data.city_mint_id)->output_resource_id != resource) {
        window_popup_dialog_show_confirmation(gettext("Change city mint production?"),
                                              gettext("Current production progress will be lost."), city_mint_conversion_changed);
    }
}
